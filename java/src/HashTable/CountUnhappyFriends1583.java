package HashTable;//You are given a list of preferences for n friends, where n is always even.
//
...
class CountUnhappyFriends1583 {
    public int unhappyFriends(int n, int[][] preferences, int[][] pairs) {
        ...
        for(int[] preference : preferences) {
            ...
            for(int i = 0; i < n - 1; i++)
                map.put(preference[i], i);
            list.add(map);
        }
        int res = 0;
        for(Integer x : pairMap.keySet()) {
            int y = pairMap.get(x);
            // int p1 = list.get(x).get(y);
            Map<Integer, Integer> map = list.get(x);
            int idx1 = map.get(y);  // x 与 y 的亲近度
            for(int i = 0; i < idx1; i++) {
                int u = preferences[x][i];  // x 与 u 的亲近度 大于 y
                Map<Integer, Integer> map2 = list.get(u);
                int v = pairMap.get(u);
                int idx2 = map2.get(x);
                int idx3 = map2.get(v);
                if(idx2 < idx3) {
